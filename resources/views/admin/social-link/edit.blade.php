...
@section('title', __('admin_localize.Edit Social Link'))
@section('content')
    <section class="section">
        <div class="section-header">
            ...
            <h1>{{__('admin_localize.Social Link')}}</h1>
        </div>
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h4>{{__('admin_localize.Update Social Link')}}</h4>
                        </div>
                        <div class="card-body">
                                ...
                                <div class="form-group row mb-4">
                                    ...
                                    <div class="col-sm-12 col-md-7">
                                      <select name="status" class="form-control selectric">
                                        ...
                                        <option {{$socialLink->status === 1 ? 'selected' : ''}} value="1">{{__('admin_localize.Active')}}</option>
                                        <option {{$socialLink->status === 0 ? 'selected' : ''}} value="0">{{__('admin_localize.InActive')}}</option>
                                      </select>
                                      @error('status')
                                        <p class="text-danger">{{$message}}</p>
                                        @enderror
